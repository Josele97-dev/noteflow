...
function WheelPicker({ items, selectedIndex, onSelect, theme }: {
  items: string[];
...
export function DateTimePickerCustom({ value, onChange, mode }: Props) {
  ...
  if (mode === 'time') {
    ...
    return (
      <View style={[styles.row, { backgroundColor: theme.background, borderRadius: 12, padding: 8 }]}>
        <WheelPicker
          items={hours}
          ...
          theme={theme}
        />
        <Text style={[styles.sep, { color: theme.text }]}>:</Text>
        <WheelPicker
          items={minutes}
  ...
  const firstDay =
    selectedYear === currentYear && selectedMonth === now.getMonth()
      ? now.getDate()
      : 1;
  ...
  const selectedDayIndex = Math.max(0, value.getDate() - firstDay);

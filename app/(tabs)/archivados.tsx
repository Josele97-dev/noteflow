...
export default function ArchivadosScreen() {
  ...
  const { _hydrated, notes, checklists, ideas, unarchiveNote, unarchiveChecklist, unarchiveIdea, deleteNote, deleteChecklist, deleteIdea } = useNotesStore();
  ...
  const sections: { label: string; items: ArchivedItem[] }[] = [
    {
      label: 'NOTAS',
      items: notes
        .filter((n) => n.archived && matches(n.title))
        .map((n) => ({ id: n.id, title: n.title, subtitle: n.content, icon: 'file-text', type: 'nota' })),
    },
    {
      label: 'TAREAS',
      items: checklists
        ...
        .map((c) => ({
          id: c.id,
          title: c.title,
          subtitle: `${c.items.filter((i: any) => i.isCompleted).length}/${c.items.length} completadas`,
          icon: 'check-circle',
          type: 'tarea',
        })),
    },
  ...
  const confirmarEliminar = (id: string, nombre: string) => {
    Alert.alert('Eliminar', `¿Seguro que quieres eliminar "${nombre}"?`, [
      ...
      { text: 'Eliminar', style: 'destructive', onPress: () => activarSalida(id) },
    ]);
  };

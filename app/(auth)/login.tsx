...
const InputField = ({ icon, placeholder, value, onChangeText, keyboardType, autoCapitalize, secureTextEntry, showPass, onTogglePass }: InputFieldProps) => {
      ...
      <View style={[styles.container, { paddingTop: insets.top + 30 }]}>
        ...
        <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
          ...
          <InputField icon="lock" placeholder="Contraseña" value={password} onChangeText={setPassword}
            secureTextEntry={!showPass} showPass={showPass} onTogglePass={() => setShowPass(p => !p)} />
          ...
          <TouchableOpacity style={[styles.btn, { backgroundColor: theme.primary }]}
            activeOpacity={0.85} onPress={login}
            accessibilityRole="button" accessibilityLabel="Iniciar sesión">
            <Text style={styles.btnText}>Iniciar sesión</Text>
          </TouchableOpacity>
        </View>

        <TouchableOpacity activeOpacity={0.7} onPress={() => router.push('/(auth)/register')}
          accessibilityRole="button" accessibilityLabel="Ir a registro">

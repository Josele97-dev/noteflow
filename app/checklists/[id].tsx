...
export default function ChecklistDetalle() {
  ...
  if (!data) return (
    <View style={{ flex: 1, backgroundColor: theme.background }}
      accessibilityRole="text" accessibilityLabel="No se encontró la lista" />
  );
  ...
  const eliminar = () => confirmar('Eliminar', '¿Seguro que quieres eliminar esta lista?', () => {
    Haptics.notificationAsync(Haptics.NotificationFeedbackType.Warning);
    deleteChecklist(id); router.back();
  });

  const archivar = () => confirmar('Archivar', '¿Quieres archivar esta lista?', () => {
    Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
    archiveChecklist(id); router.back();
  });
  ...
  return (
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent}
        showsVerticalScrollIndicator={false} accessibilityLabel="Detalle de checklist">
        ...
        <FadeInDown duration={400} offset={-30} delay={100}>
          <Text style={[styles.title, { color: theme.text }]}
            accessibilityRole="header" accessibilityLabel={`Título de la lista: ${data.title}`}>{data.title}</Text>
        </FadeInDown>

        <FadeInDown duration={400} offset={-30} delay={200}>
          <View style={styles.itemsContainer}>
            {data.items.map((item) => (
              <TouchableOpacity key={item.id} style={styles.itemRow} activeOpacity={0.7}
                onPress={() => toggleChecklistItem(data.id, item.id)}
                accessibilityRole="button"
                accessibilityLabel={`${item.text}, ${item.isCompleted ? 'completado' : 'pendiente'}`}
                accessibilityHint="Marca o desmarca esta tarea"
                accessibilityState={{ checked: item.isCompleted }}>
                <View style={[styles.checkbox, { borderColor: theme.border, backgroundColor: item.isCompleted ? theme.primary : 'transparent' }]} />
                <Text style={[styles.itemText, { color: theme.textSecondary, textDecorationLine: item.isCompleted ? 'line-through' : 'none' }]}>
                  {item.text}
                </Text>
              </TouchableOpacity>
            ))}
        ...
        {data.location && (
          <FadeInDown duration={400} offset={-30} delay={300}>
            <TouchableOpacity style={[styles.locationRow, { backgroundColor: theme.card, borderColor: theme.border }]}

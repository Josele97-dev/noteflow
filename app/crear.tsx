...
const COLOR_NAMES: Record<string, string> = {
  '#FFD700': 'amarillo', '#FF6B6B': 'rojo coral', '#6C63FF': 'morado',
  '#4CAF50': 'verde', '#FF9800': 'naranja', '#00BCD4': 'turquesa',
};
...
async function getCurrentLocation(): Promise<NoteLocation | null> {
  try {
    ...
    return {
      ...
      address: [addr.street, addr.city].filter(Boolean).join(', ') || 'Ubicación desconocida',
    };
  } catch { return null; }
}

const TIPOS = ['nota', 'tarea', 'idea'] as const;
const COLORES = ['#FFD700', '#FF6B6B', '#6C63FF', '#4CAF50', '#FF9800', '#00BCD4'] as const;
...
type AddRowProps = { value: string; onChange: (t: string) => void; onAdd: () => void; placeholder: string; theme: ReturnType<typeof useTheme> };
...
const AddRow = ({ value, onChange, onAdd, placeholder, theme }: AddRowProps) => (
  <View style={styles.row}>
    <TextInput
      ...
      accessibilityLabel={placeholder} accessibilityRole="text"
    />
    <TouchableOpacity style={[styles.addBtn, { backgroundColor: theme.primary }]} onPress={onAdd} accessibilityRole="button" accessibilityLabel={`Añadir ${placeholder}`}>
...
export default function NuevaNota() {
  ...
  const [tipo, setTipo] = useState<typeof TIPOS[number]>(
    (TIPOS as readonly string[]).includes(tipoParam ?? '') ? (tipoParam as typeof TIPOS[number]) : 'nota'
  );
  ...
  const resetForm = () => {
    setErrors({}); setTitle(''); setContent('');
    setTareas([]); setTareaInput(''); setTags([]); setTagInput(''); setColor('#FFD700');
  };
  ...
  const guardar = async () => {
    ...
    if (tipo === 'tarea' && tareas.length === 0) {
      setErrors({ tareas: ['Debes añadir al menos una tarea'] });
      return setIsSaving(false);
    }

    const result = (tipo === 'nota' ? noteSchema : baseSchema).safeParse({ title, content });
    if (!result.success) {
      setIsSaving(false);
    ...
    try {
      ...
      if (tipo === 'nota') {
        ...
        created = await addChecklist({ title, items: tareas.map((t, i) => ({ id: `${Date.now()}-${i}`, text: t, isCompleted: false })) });
      } else {
  ...
  return (
    <KeyboardAvoidingView style={{ flex: 1, backgroundColor: theme.background }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={[styles.header, { backgroundColor: isDark ? theme.card : theme.primary, paddingTop: insets.top + 12 }]}>
        <TouchableOpacity onPress={() => router.back()} style={styles.backBtn} accessibilityRole="button" accessibilityLabel="Volver atrás">
          <Ionicons name="arrow-back-circle" size={28} color={isDark ? theme.text : '#fff'} />
        </TouchableOpacity>
        <Text style={[styles.headerTitle, { color: isDark ? theme.text : '#fff' }]} accessibilityRole="header">Nueva entrada</Text>
        <TouchableOpacity
          onPress={guardar} disabled={isSaving}
          style={[styles.saveBtn, { backgroundColor: isDark ? theme.primary : 'rgba(255,255,255,0.2)', opacity: isSaving ? 0.5 : 1 }]}
          accessibilityRole="button" accessibilityLabel={isSaving ? 'Guardando' : 'Guardar entrada'} accessibilityState={{ disabled: isSaving }}
        >
      ...
      <ScrollView style={styles.container} contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        <View style={styles.tipos}>
          {TIPOS.map((t) => (
            <TouchableOpacity key={t}
              ...
              onPress={() => { setTipo(t); resetForm(); }}
              accessibilityRole="button" accessibilityLabel={`Seleccionar tipo ${t}`} accessibilityState={{ selected: tipo === t }}
            >
        ...
        <TextInput style={[...inputStyle, errors.title && { borderColor: theme.danger }]}
          ...
          value={title} onChangeText={setTitle} accessibilityLabel="Título de la entrada"
        />
        ...
        {tipo === 'nota' && (
          <>
            <TextInput style={[...inputStyle, styles.textarea, errors.content && { borderColor: theme.danger }]}
              ...
              value={content} onChangeText={setContent} multiline accessibilityLabel="Contenido de la nota"
            />
        ...
        {tipo === 'tarea' && (
          <>
            ...
            />
            {errors.tareas && <Text style={[styles.error, { color: theme.danger }]} accessibilityRole="alert">{errors.tareas[0]}</Text>}
            {tareas.map((t, i) => (
              <View key={i} style={styles.itemRow}>
                <Text style={[styles.item, { color: theme.textSecondary }]} accessibilityRole="text">• {t}</Text>
                <TouchableOpacity onPress={() => setTareas((p) => p.filter((_, idx) => idx !== i))} accessibilityRole="button" accessibilityLabel={`Eliminar tarea ${t}`}>
        ...
        {tipo === 'idea' && (
          <>
            ...
            <View style={styles.tags}>
              {tags.map((tag, i) => (
                <TouchableOpacity key={i} style={[styles.tag, { backgroundColor: theme.primary + '22' }]}
                  ...
                  accessibilityRole="button" accessibilityLabel={`Eliminar etiqueta ${tag}`}
                >
            ...
            <View style={styles.colores}>
              {COLORES.map((c) => (
                <TouchableOpacity key={c}
                  ...
                  onPress={() => setColor(c)}
                  accessibilityRole="button" accessibilityLabel={`Seleccionar color ${COLOR_NAMES[c]}`} accessibilityState={{ selected: color === c }}
                />
              ))}
            </View>
            ...
            <TextInput style={[...inputStyle, styles.textarea]}
              ...
              value={content} onChangeText={setContent} multiline accessibilityLabel="Descripción de la idea"
            />
          </>
        )}
      </ScrollView>
      ...
      <Modal visible={showReminderModal} transparent animationType="fade"
        onRequestClose={() => handleReminder(false)} onDismiss={() => handleReminder(false)} accessibilityViewIsModal
      >
        <View style={styles.modalOverlay} accessibilityRole="alert" accessibilityLabel="Añadir recordatorio">
          <View style={[styles.modalBox, { backgroundColor: theme.card }]}>
            <Text style={[styles.modalTitle, { color: theme.text }]} accessibilityRole="header">¿Añadir recordatorio?</Text>
            <Text style={[styles.modalSub, { color: theme.textSecondary }]} accessibilityRole="text">{`Te notificaremos para "${title}"`}</Text>

            {dateError !== '' && (
              <Text style={{ color: theme.danger, fontWeight: '600' }} accessibilityRole="alert">{dateError}</Text>
            )}
            ...
            <View style={styles.row}>
              <TouchableOpacity
                ...
                accessibilityRole="button" accessibilityLabel="Seleccionar fecha" accessibilityState={{ selected: pickerMode === 'date' }}
              >
              ...
              <TouchableOpacity
                ...
                accessibilityRole="button" accessibilityLabel="Seleccionar hora" accessibilityState={{ selected: pickerMode === 'time' }}
              >
                <Text style={[styles.dateLabel, { color: theme.textSecondary }]}>🕐 Hora</Text>
                <Text style={[styles.dateValue, { color: theme.text }]}>{reminderDate.toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}</Text>
              </TouchableOpacity>
            </View>

            {pickerMode && <DateTimePickerCustom value={reminderDate} mode={pickerMode} onChange={handleDateChange} />}

            <View style={styles.modalActions}>
              <TouchableOpacity style={[styles.modalBtn, { borderColor: theme.border }]} onPress={() => handleReminder(false)}
                accessibilityRole="button" accessibilityLabel="Guardar sin recordatorio"
              >
              ...
              <TouchableOpacity
                ...
                accessibilityRole="button" accessibilityLabel="Añadir recordatorio" accessibilityState={{ disabled: !isDateValid }}
              >
